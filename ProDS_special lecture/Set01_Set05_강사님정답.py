...
# q3['Sex_cd']=np.where(q3.Sex.isna(), q3.Sex, np.where(q3.Sex == 'M', 0, 1))
# q3['Sex_cd']=np.where(q3.Sex.isna(), np.nan, np.where(q3.Sex == 'M', 0, 1))
...
# - 비율(forehead_ratio) 평균: xbar, 비율(forehead_ratio) 표준편차: std
...
# - 이상치 수
((q1['forehead_ratio'] < LB) | (q1['forehead_ratio'] > UB)).sum() # 3
...
# 관측 데이터 타입: 수치형
# - 그룹수: 1 -> 일표본 t 검정  
#    H0: mu == 10, H1: mu != 10 (양측)
#    H0: mu >= 10, H1: mu < 10 (하단측)
#    H0: mu <= 10, H1: mu > 10 (상단측)

# - 그룹수: 2 -> 이표본 t 검정(독립, 대응)
# -- 대응: 그룹별로 데이터 수가 동일, 순서가 일치
# -- 독립: 그룹별로 데이터 수, 순서 일치하지 않아도 됨(분산고려)

#    H0: mu1 == mu2, H1: mu != mu2 (양측) => mu1 - mu2 = 0
#    H0: mu1 >= mu2, H1: mu1 < mu2 (하단측)
#    H0: mu1 <= mu2, H1: mu1 > mu2 (상단측)
...
from scipy.stats import ttest_ind, bartlett, levene # 등분산검정용 함수
...
# (2) 독립인 2표본 t 검정 수행
ttest_out=ttest_ind(q2_m, q2_f, equal_var=False)
dir(ttest_out)
...
q3['TIME'].ndim
q3['TIME'].shape
q3['TIME'].values.reshape(-1, 2).shape
