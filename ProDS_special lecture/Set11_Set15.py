...
# 방법 2 
q1 = pd.pivot_table(data11, index='Country', columns = 'year', values = 'Happiness_Score').dropna()
...
q1['ratio'] = (q1[2017] - q1[2015]) / 2

# 행복지수 증감률이 가장 높은 3개 국가를 행복지수가
# 높은 순서대로 차례대로 기술

q1['ratio'].nlargest(3).index
...
anova_out1 = f_oneway(q1[2015], q1[2016], q1[2017])
...
q3 = q1[[2015, 2016, 2017]].melt()

anova_out2 = ols('value ~ C(year)', q3).fit()
anova_lm(anova_out2)['F'][0]
# 답 : 0.0042

# 사후분석은 집단간에 차이가 날 경우 해야함[다중비교]
...
q1 = pos1[pos1.Date == sel_date]

q1.itemDescription.value_counts().max()
q1.itemDescription.value_counts().idxmax()
...
# 2. 데이터 결합
merge1 = pd.merge(pos2, list1, left_on= 'itemDescription', right_on= 'prod_nm', 
                  how = 'left')
...
# 4. 일별 주류 판매 집계
q2 = pd.pivot_table(merge2, index='Date',
               columns='week',
               values='alcohol',
               aggfunc='sum')
...
ttest_ind(q2[1].dropna(), q2[0].dropna(), equal_var=False)
